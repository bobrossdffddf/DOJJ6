...
for (const d of [
  DATA_DIR,
  UPLOADS_DIR,
  WARRANT_REQ_UPLOADS_DIR,
  WARRANT_RETURN_UPLOADS_DIR,
  WARRANT_PDFS_DIR,
])
...
function generateWarrantPdf(warrantData) {
  return new Promise((resolve) => {
    ...
    py.on("close", (code) => {
      if (code !== 0) {
        console.error("[PDF] Failed:", err.trim());
        return resolve(null);
      }
      resolve(outFile);
    });
    py.on("error", (e) => {
      console.error("[PDF] spawn error:", e.message);
      resolve(null);
    });
  });
}
...
const wrUploadStorage = multer.diskStorage({
  destination(req, file, cb) {
    cb(null, WARRANT_REQ_UPLOADS_DIR);
  },
  filename(req, file, cb) {
    const safe = file.originalname.replace(/[^a-zA-Z0-9._\-]/g, "_");
    cb(null, `${Date.now()}_${safe}`);
...
const wrUpload = multer({
  storage: wrUploadStorage,
  limits: { fileSize: 250 * 1024 * 1024 },
});

const wrReturnStorage = multer.diskStorage({
  destination(req, file, cb) {
    cb(null, WARRANT_RETURN_UPLOADS_DIR);
  },
  filename(req, file, cb) {
    const safe = file.originalname.replace(/[^a-zA-Z0-9._\-]/g, "_");
    cb(null, `${Date.now()}_${safe}`);
...
const wrReturnUpload = multer({
  storage: wrReturnStorage,
  limits: { fileSize: 250 * 1024 * 1024 },
});
...
function loadRoleConfig() {
  try {
    return JSON.parse(fs.readFileSync(ROLE_CONFIG_FILE, "utf8"));
  } catch {
    return JSON.parse(JSON.stringify(DEFAULT_ROLE_KEYWORDS));
  }
}
...
async function fetchMemberInfo(userId) {
  if (!config.botToken || !config.guildId)
    return { roleIds: [], roleNames: [], isAdmin: false };
...
function nextSubpoenaNumber() {
  ...
  const nums = subs.map((s) => {
    const m = String(s.subpoenaNumber || "").match(/(\d+)$/);
    return m ? parseInt(m[1]) : 0;
  });
...
function nextWarrantRequestNumber() {
  ...
  const year = new Date().getFullYear();
  const nums = reqs.map((r) => {
    const m = String(r.requestNumber || "").match(/(\d+)$/);
    return m ? parseInt(m[1]) : 0;
  });
...
const PERM_LEVEL_CLASS = {
  citizen: "badge-gray",
  clerk: "badge-blue",
  lawyer: "badge-purple",
  ag: "badge-green",
};
const PERM_LEVEL_LABEL = {
  citizen: "Citizen",
  clerk: "Court Clerk",
  lawyer: "Attorney / ADA",
  ag: "Attorney General",
};
const SUBPOENA_STATUS_CLASS = {
  pending: "badge-yellow",
  served: "badge-green",
  failed: "badge-red",
  quashed: "badge-gray",
...
const PLEA_CLASS = {
  "not entered": "badge-gray",
  "not guilty": "badge-green",
  guilty: "badge-red",
  "no contest": "badge-yellow",
};
const VERDICT_CLASS = {
  pending: "badge-yellow",
  "not guilty": "badge-green",
...
app.get("/api/cases", (req, res) => {
  ...
  if (key !== process.env.BOT_API_KEY && process.env.BOT_API_KEY)
  ...
  res.json(
    status === "all"
      ? cases
      : cases.filter((c) => !["closed", "dismissed"].includes(c.status)),
  );
});
...
app.get("/api/members", ensureAuth, async (req, res) => {
  ...
  if (!q || q.length < 2 || !config.botToken || !config.guildId)
    return res.json([]);
  try {
    const r = await fetch(
...
app.get("/", (req, res) => {
  ...
  <div class="login-wrap">
    <div class="login-card">
      ...
      <p class="login-sub">State of Texas — Department of Justice<br/>Sign in with your Discord account to gain access to the portal.</p>
      ${
        ready
  ...
  return res.send(
    layout({ req, title: "Sign In — DOJ Portal", body, user: null }),
  );
});
...
app.get("/auth/discord/callback", async (req, res) => {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return res.status(401).send(`OAuth error: ${escapeHtml(t)}`);
    }
    ...
    if (!userRes.ok)
      return res.status(401).send("Could not fetch Discord profile.");
    ...
    if (config.guildId) {
      ...
      if (!gRes.ok)
        return res.status(403).send("Could not verify guild membership.");
      const guilds = await gRes.json();
      if (!guilds.some((g) => g.id === config.guildId))
        return res
          .status(403)
          .send("Access denied: not a member of the configured server.");
    }
    ...
    if (config.botToken && config.guildId) {
      try {
        ...
        /* bot unavailable, default to citizen */
      }
    } else {
      permLevel = "ag";
...
app.get("/refresh-roles", ensureAuth, async (req, res) => {
  if (config.botToken && config.guildId) {
    try {
      ...
      req.session.user.permLevel = info.isAdmin
        ? "ag"
        : detectPermLevel(info.roleNames);
    } catch {}
  }
  return res.redirect("/dashboard");
});
...
app.get("/dashboard", ensureAuth, (req, res) => {
  ...
  const pendingTrial = cases.filter((c) =>
    ["pending", "filed"].includes(c.status),
  ).length;
  const activeWarrants = warrants.filter((w) => w.status === "active").length;
  const pendingSubpoenas = subpoenas.filter(
    (s) => s.status === "pending",
  ).length;
  ...
  if (pl === "citizen") {
    ...
    <div class="page-header">
      ...
      <p class="page-sub">Signed in as ${escapeHtml(user.username)} · State of Texas Department of Justice</p>
    </div>
    ${permBanner}
    <div class="card">
    ...
    return res.send(
      layout({ req, title: "Dashboard — DOJ", body, user, page: "dashboard" }),
    );
  }
  ...
  <div class="activity-row">
    ...
    <div class="activity-info">
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1rem">No recent activity.</p>';
  ...
  <a class="table-row-link" href="/cases/${c.id}" style="--cols:3">
    ...
    <span class="tr-cell">${badge(c.status, CASE_STATUS_CLASS[c.status] || "badge-gray")}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1rem">No cases yet.</p>';
  ...
  <div class="activity-row">
    ...
    <div class="activity-info">
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1rem">No upcoming hearings.</p>';
  ...
  return (
    {
      ...
      case_closed: "CLO",
      warrant_issued: "WRT",
      warrant_executed: "EXE",
      file_uploaded: "DOC",
      note_added: "NOTE",
      subpoena_issued: "SUB",
      defendant_added: "DEF",
      evidence_added: "EVD",
      sentence_recorded: "SEN",
    }[type] || "–"
  );
}
...
app.get("/cases", requirePerm("clerk"), (req, res) => {
  ...
  <a class="table-row-link" href="/cases/${c.id}">
    ...
    <span class="tr-cell muted-text">${fmtDate(c.createdAt)}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1.5rem">No cases match your filters.</p>';

  const countyOptions = TEXAS_COUNTIES.map(
    (cn) =>
      `<option value="${cn}" ${county === cn ? "selected" : ""}>${cn}</option>`,
  ).join("");
  const gradeOptions = CASE_GRADES.map(
    (g) =>
...
app.get("/cases/new", requirePerm("clerk"), (req, res) => {
  const chargeOptions = COMMON_CHARGES.map(
    (c) => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`,
  ).join("");
  const countyOptions = TEXAS_COUNTIES.map(
    (cn) => `<option value="${cn}">${cn}</option>`,
  ).join("");
  const courtOptions = COURT_TYPES.map(
    (ct) => `<option value="${escapeHtml(ct)}">${escapeHtml(ct)}</option>`,
  ).join("");
  const gradeOptions = CASE_GRADES.map(
...
app.post("/cases", requirePerm("clerk"), (req, res) => {
  ...
  logActivity(
    "case_created",
    `Case ${newCase.caseNumber} created — ${title}`,
    req.session.user.username,
  );

  if (!["civil", "internal affairs"].includes(type)) {
    const defendants = readJSON(DEFENDANTS_FILE);
    const exists = defendants.some(
      (d) => d.fullName.toLowerCase() === subject.toLowerCase(),
    );
    if (!exists) {
      defendants.unshift({
        id: newId(),
      ...
      logActivity(
        "defendant_added",
        `Defendant record auto-created: ${subject}`,
        req.session.user.username,
      );
    }
  }
...
app.get("/cases/:id", requirePerm("clerk"), (req, res) => {
  ...
  const pl = req.session.user.permLevel;
  const linkedWarrants = readJSON(WARRANTS_FILE).filter(
    (w) => w.linkedCaseId === c.id,
  );
  const linkedSubpoenas = readJSON(SUBPOENAS_FILE).filter(
    (s) => s.linkedCaseId === c.id,
  );
  ...
  const chargesHtml =
    (c.charges || [])
      .map((ch) => `<span class="tag">${escapeHtml(ch)}</span>`)
      .join("") || '<span class="muted-text">None listed</span>';
  const notesHtml =
    (c.caseNotes || [])
      .map(
        (n) => `
  ...
  <a class="table-row-link" href="/warrants/${w.id}" style="--cols:4">
    ...
    <span class="tr-cell muted-text">${fmtDate(w.issuedAt)}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:0.75rem">No warrants linked.</p>';
  ...
  <div class="table-row-link" style="--cols:4">
    ...
    <span class="tr-cell muted-text">${fmtDate(s.dueDate)}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:0.75rem">No subpoenas issued.</p>';
  ...
  <div class="card">
    ...
    ${(c.exhibits || [])
      .map((e) => {
        ...
        const statusLabel =
          { admitted: "Admitted", rejected: "Rejected", pending: "Pending" }[
            e.status
          ] || "Pending";
        return `
  ...
  <div class="card">
    ...
    <div class="table-rows">
      ...
      <div class="table-row-link" style="--cols:5">
        ...
        : '<p class="muted-text" style="padding:0.75rem 0">No evidence logged yet.</p>'
    }
    ${
      canWrite
        ? `
...
app.get("/cases/:id/edit", requirePerm("clerk"), (req, res) => {
  ...
  if (!c) return res.status(404).send("Case not found.");
  const chargeOptions = COMMON_CHARGES.map(
    (ch) => `<option value="${escapeHtml(ch)}">${escapeHtml(ch)}</option>`,
  ).join("");
  const statusOptions = [
    "open",
    "investigation",
  ...
  const countyOptions = TEXAS_COUNTIES.map(
    (cn) =>
      `<option value="${cn}" ${c.county === cn ? "selected" : ""}>${cn}</option>`,
  ).join("");
  const courtOptions = COURT_TYPES.map(
    (ct) =>
      `<option value="${escapeHtml(ct)}" ${c.courtType === ct ? "selected" : ""}>${escapeHtml(ct)}</option>`,
  ).join("");
  const gradeOptions = CASE_GRADES.map(
    (g) =>
      `<option value="${escapeHtml(g)}" ${c.caseGrade === g ? "selected" : ""}>${escapeHtml(g)}</option>`,
  ).join("");
  const pleaOptions = ["not entered", "not guilty", "guilty", "no contest"]
    .map(
      (p) =>
        `<option value="${p}" ${(c.plea || "not entered") === p ? "selected" : ""}>${p.charAt(0).toUpperCase() + p.slice(1)}</option>`,
    )
    .join("");
  const verdictOptions = [
    ...
    .map(
      (v) =>
        `<option value="${v}" ${(c.verdict || "pending") === v ? "selected" : ""}>${v.charAt(0).toUpperCase() + v.slice(1)}</option>`,
    )
    .join("");
...
app.post("/cases/:id/edit", requirePerm("clerk"), (req, res) => {
  ...
  logActivity(
    "case_updated",
    `Case ${cases[idx].caseNumber} updated`,
    req.session.user.username,
  );
  refreshBotEmbeds();
  if (_bot && _bot.notifyCaseUpdate)
    _bot
      .notifyCaseUpdate(req.params.id, req.session.user.username)
      .catch(() => {});
...
app.post("/cases/:id/notes", requirePerm("clerk"), (req, res) => {
  ...
  logActivity(
    "note_added",
    `Note added to case ${cases[idx].caseNumber}`,
    req.session.user.username,
  );
  return res.redirect(`/cases/${req.params.id}`);
});
...
app.post("/cases/:id/evidence", requirePerm("lawyer"), (req, res) => {
  ...
  const idx = cases.findIndex((x) => x.id === req.params.id);
  if (idx === -1) return res.status(404).send("Not found.");
  const { item, type, description, collectedBy, collectedAt, locationFound } =
    req.body;
  ...
  logActivity(
    "evidence_added",
    `Evidence logged on case ${cases[idx].caseNumber}: ${item.trim()}`,
    req.session.user.username,
  );
  return res.redirect(`/cases/${req.params.id}`);
});
...
app.post(
  "/cases/:id/evidence/:evidenceId/delete",
  requirePerm("lawyer"),
  (req, res) => {
    const cases = readJSON(CASES_FILE);
    const idx = cases.findIndex((x) => x.id === req.params.id);
    if (idx === -1) return res.status(404).send("Not found.");
    cases[idx].evidence = (cases[idx].evidence || []).filter(
      (e) => e.id !== req.params.evidenceId,
    );
    cases[idx].updatedAt = new Date().toISOString();
    writeJSON(CASES_FILE, cases);
    return res.redirect(`/cases/${req.params.id}`);
...
app.post(
  "/cases/:id/exhibits/:exhibitId/status",
  requirePerm("lawyer"),
  (req, res) => {
    const cases = readJSON(CASES_FILE);
    const idx = cases.findIndex((x) => x.id === req.params.id);
    if (idx === -1) return res.status(404).send("Not found.");
    const exhibit = (cases[idx].exhibits || []).find(
      (e) => e.id === req.params.exhibitId,
    );
    ...
    logActivity(
      "exhibit_updated",
      `Exhibit ${exhibit.letter} in ${cases[idx].caseNumber} marked ${status}`,
      req.session.user.username,
    );
    if (_bot && _bot.updateExhibitEmbed)
      _bot
        .updateExhibitEmbed(req.params.id, req.params.exhibitId, status)
        .catch(() => {});
    return res.redirect(`/cases/${req.params.id}`);
  },
);
...
app.post(
  "/cases/:id/exhibits/:exhibitId/describe",
  requirePerm("clerk"),
  (req, res) => {
    const cases = readJSON(CASES_FILE);
    const idx = cases.findIndex((x) => x.id === req.params.id);
...
app.post(
  "/cases/:id/exhibits/:exhibitId/delete",
  requirePerm("ag"),
  (req, res) => {
    const cases = readJSON(CASES_FILE);
    const idx = cases.findIndex((x) => x.id === req.params.id);
    if (idx === -1) return res.status(404).send("Not found.");
    cases[idx].exhibits = (cases[idx].exhibits || []).filter(
      (e) => e.id !== req.params.exhibitId,
    );
    cases[idx].updatedAt = new Date().toISOString();
    writeJSON(CASES_FILE, cases);
...
app.post(
  "/cases/:id/courtdocs/:docId/delete",
  requirePerm("ag"),
  (req, res) => {
    const cases = readJSON(CASES_FILE);
    const idx = cases.findIndex((x) => x.id === req.params.id);
    if (idx === -1) return res.status(404).send("Not found.");
    cases[idx].courtDocuments = (cases[idx].courtDocuments || []).filter(
...
app.get("/warrants", requirePerm("citizen"), (req, res) => {
  ...
  if (q)
    warrants = warrants.filter((w) =>
      [w.warrantNumber, w.subject, w.issuedBy, w.description, w.judge]
        .join(" ")
        .toLowerCase()
        .includes(q.toLowerCase()),
    );
  if (status && hasPerm(pl, "clerk"))
    warrants = warrants.filter((w) => w.status === status);
  ...
  <a class="table-row-link" href="/warrants/${w.id}" style="--cols:5">
    ...
    <span class="tr-cell muted-text">${fmtDate(w.issuedAt)}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1.5rem">No warrants match your search.</p>';

  const citizenNotice = !hasPerm(pl, "clerk")
    ? `<div class="alert-box">Showing <strong>active warrants only</strong>. Sign in with a staff role to access all warrant records.</div>`
    : "";
...
app.post("/warrants", requirePerm("clerk"), async (req, res) => {
  ...
  if (!type || !subject || !county || !judge || !description)
    return res.status(400).send("Missing required fields.");
  const warrants = readJSON(WARRANTS_FILE);
  const w = {
    id: newId(),
    warrantNumber: nextWarrantNumber(),
    type,
    subject,
    county: county || "",
    judge: judge || "",
    ...
    address:
      type === "search" ? searchProperty || address || "" : address || "",
    ...
    agency: agency || "State of Texas DOJ",
    itemsToSeize: itemsToSeize || "",
    executeBy: executeBy || "",
  ...
  logActivity(
    "warrant_issued",
    `${type.charAt(0).toUpperCase() + type.slice(1)} Warrant ${w.warrantNumber} issued for ${subject}`,
    req.session.user.username,
  );
...
app.get("/warrants/:id", requirePerm("citizen"), (req, res) => {
  ...
  const pl = req.session.user.permLevel;
  if (!hasPerm(pl, "clerk") && w.status !== "active")
    return res.status(403).send("Access denied.");
  const canWrite = hasPerm(pl, "lawyer");
  const canDelete = hasPerm(pl, "ag");

  const linkedCase = w.linkedCaseId
    ? readJSON(CASES_FILE).find((c) => c.id === w.linkedCaseId)
    : null;
...
app.post(
  "/warrants/:id/execute",
  requirePerm("lawyer"),
  wrReturnUpload.single("returnDoc"),
  (req, res) => {
    const warrants = readJSON(WARRANTS_FILE);
    ...
    logActivity(
      "warrant_executed",
      `Warrant ${warrants[idx].warrantNumber} executed by ${req.session.user.username}`,
      req.session.user.username,
    );
    refreshBotEmbeds();
...
app.post("/warrants/:id/cancel", requirePerm("lawyer"), (req, res) => {
  ...
  logActivity(
    "warrant_executed",
    `Warrant ${warrants[idx].warrantNumber} cancelled`,
    req.session.user.username,
  );
  refreshBotEmbeds();
  return res.redirect(`/warrants/${req.params.id}`);
...
app.get("/defendants", requirePerm("clerk"), (req, res) => {
  ...
  if (q)
    defendants = defendants.filter((d) =>
      [d.fullName, d.dob, d.address, d.notes]
        .join(" ")
        .toLowerCase()
        .includes(q.toLowerCase()),
    );
  ...
  <a class="table-row-link" href="/defendants/${d.id}" style="--cols:4">
    ...
    <span class="tr-cell muted-text">${fmtDate(d.createdAt)}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1.5rem">No defendants on record.</p>';
...
app.post("/defendants", requirePerm("lawyer"), (req, res) => {
  ...
  logActivity(
    "defendant_added",
    `Defendant record added: ${fullName}`,
    req.session.user.username,
  );
...
app.get("/defendants/:id", requirePerm("clerk"), (req, res) => {
  ...
  const pl = req.session.user.permLevel;
  const linkedCases = readJSON(CASES_FILE).filter(
    (c) => c.subject && c.subject.toLowerCase() === d.fullName.toLowerCase(),
  );
  const linkedWarrants = readJSON(WARRANTS_FILE).filter(
    (w) => w.subject && w.subject.toLowerCase() === d.fullName.toLowerCase(),
  );
  ...
  <a class="table-row-link" href="/cases/${c.id}" style="--cols:3">
    ...
    <span class="tr-cell">${badge(c.status, CASE_STATUS_CLASS[c.status] || "badge-gray")}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:0.75rem">No linked cases found.</p>';
  ...
  <a class="table-row-link" href="/warrants/${w.id}" style="--cols:3">
    ...
    <span class="tr-cell">${badge(w.status, WARRANT_STATUS_CLASS[w.status] || "badge-gray")}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:0.75rem">No linked warrants found.</p>';
...
app.get("/subpoenas", requirePerm("clerk"), (req, res) => {
  ...
  if (q)
    subpoenas = subpoenas.filter((s) =>
      [s.subpoenaNumber, s.recipient, s.issuedBy, s.purpose]
        .join(" ")
        .toLowerCase()
        .includes(q.toLowerCase()),
    );
  ...
  <a class="table-row-link" href="/subpoenas/${s.id}" style="--cols:5">
    ...
    <span class="tr-cell muted-text">${fmtDate(s.dueDate)}</span>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1.5rem">No subpoenas found.</p>';
...
app.post("/subpoenas", requirePerm("lawyer"), (req, res) => {
  ...
  if (!recipient || !dueDate || !purpose)
    return res.status(400).send("Missing required fields.");
  const subpoenas = readJSON(SUBPOENAS_FILE);
  const s = {
    id: newId(),
    subpoenaNumber: nextSubpoenaNumber(),
    recipient,
    type: type || "testimony",
    linkedCaseId: linkedCaseId || "",
    dueDate,
    location: location || "",
    issuedBy: issuedBy || req.session.user.username,
    purpose,
    status: "pending",
    createdAt: new Date().toISOString(),
  };
  ...
  logActivity(
    "subpoena_issued",
    `Subpoena ${s.subpoenaNumber} issued to ${recipient}`,
    req.session.user.username,
  );
...
app.get("/subpoenas/:id", requirePerm("clerk"), (req, res) => {
  ...
  const pl = req.session.user.permLevel;
  const linkedCase = s.linkedCaseId
    ? readJSON(CASES_FILE).find((c) => c.id === s.linkedCaseId)
    : null;
  const canWrite = hasPerm(pl, "lawyer");
...
app.get("/calendar", requirePerm("clerk"), (req, res) => {
  ...
  const hearings = cases
    .filter(
      (c) =>
        c.courtDate &&
        new Date(c.courtDate) >= monthStart &&
        new Date(c.courtDate) <= monthEnd,
    )
    .sort((a, b) => new Date(a.courtDate) - new Date(b.courtDate));
  const trials = cases
    .filter(
      (c) =>
        c.trialDate &&
        new Date(c.trialDate) >= monthStart &&
        new Date(c.trialDate) <= monthEnd,
    )
  ...
  <div class="calendar-event hearing">
    ...
    <div class="cal-event-body">
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1rem">No hearings scheduled this month.</p>';
  ...
  <div class="calendar-event trial">
    ...
    <div class="cal-event-body">
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1rem">No trials scheduled this month.</p>';
...
app.get("/channels", requirePerm("clerk"), async (req, res) => {
  ...
  let channels = [];
  try {
    channels = await getAccessibleChannels(
      user.id,
      user.roleIds || [],
      user.isAdmin || false,
    );
  ...
  if (!channels.length) {
    ...
    return res.send(
      layout({ req, title: "Documents — DOJ", body, user, page: "channels" }),
    );
  }
...
app.get("/channels/:channelId", requirePerm("clerk"), async (req, res) => {
  ...
  const fileRows = files
    .map((f) => {
      const ext =
        path.extname(f.originalName).replace(".", "").toUpperCase() || "FILE";
      return `
    <div class="file-row">
...
app.post(
  "/channels/:channelId/files/:filename/delete",
  requirePerm("clerk"),
  (req, res) => {
    const fp = path.join(
      UPLOADS_DIR,
...
app.get("/warrant-request", ensureAuth, (req, res) => {
  ...
  <div class="request-card">
    <div class="request-info">
      ...
      ${
        r.reviewedBy
          ? `<div class="request-reviewer" style="margin-top:0.4rem;font-size:0.8rem;color:#6b7280">
    ...
    <div class="request-actions">${badge(r.status, WR_STATUS_CLASS[r.status] || "badge-gray")}</div>
      ...
      .join("") ||
    '<p class="muted-text" style="padding:0.5rem 0">You have not submitted any warrant requests yet.</p>';
  ...
  return res.send(
    layout({
      req,
      title: "Request Warrant — DOJ",
      body,
      user,
      page: "warrant-request",
    }),
  );
});
...
app.post(
  ...
  (req, res) => {
    ...
    logActivity(
      "warrant_issued",
      `Warrant request ${record.requestNumber} submitted for ${subject}`,
      user.username,
    );
...
app.get("/warrant-requests", requirePerm("clerk"), (req, res) => {
  ...
  <div class="request-card">
    ...
    <div class="request-actions" style="flex-direction:column;align-items:flex-end;gap:0.5rem;">
      ...
      ${
        hasPerm(pl, "clerk")
          ? `<form method="post" action="/warrant-requests/${r.id}/delete" onsubmit="return confirm('Delete this request? This cannot be undone.')">
      ...
      .join("") ||
    '<p class="muted-text" style="padding:1rem">No requests match your filter.</p>';

  const pending = readJSON(WARRANT_REQUESTS_FILE).filter(
    (r) => r.status === "pending",
  ).length;
  const body = `
  <div class="page-header row-between">
  ...
  return res.send(
    layout({
      req,
      title: "Warrant Requests — DOJ",
      body,
      user: req.session.user,
      page: "warrant-requests",
    }),
  );
});

app.post(
  "/warrant-requests/:id/approve",
  requirePerm("clerk"),
  async (req, res) => {
    const reqs = readJSON(WARRANT_REQUESTS_FILE);
    ...
    const w = {
      ...
      subject: r.subject,
      county: r.county,
      judge: req.session.user.username,
      subjectDob: r.subjectDob || "",
      subjectDescription: r.subjectDescription || "",
      address: r.searchProperty || r.address || "",
      linkedCaseId: "",
      status: "active",
      issuedBy: req.session.user.username,
      ...
      eyes: r.eyes || "",
      phone: r.phone || "",
      itemsToSeize: r.itemsToSeize || "",
      documentType: "Complaint",
      agency: "State of Texas DOJ",
    };
    warrants.unshift(w);
    writeJSON(WARRANTS_FILE, warrants);
    ...
    logActivity(
      "warrant_issued",
      `Warrant ${w.warrantNumber} signed & approved from request ${r.requestNumber} — ${r.subject}`,
      req.session.user.username,
    );
    refreshBotEmbeds();
...
app.post("/warrant-requests/:id/delete", requirePerm("clerk"), (req, res) => {
  ...
  logActivity(
    "warrant_deleted",
    `Warrant request deleted by ${req.session.user.username}`,
    req.session.user.username,
  );
  return res.redirect("/warrant-requests");
});
...
app.get("/search", ensureAuth, (req, res) => {
  ...
  if (lq) {
    if (hasPerm(pl, "clerk")) {
      caseResults = readJSON(CASES_FILE)
        .filter((c) =>
          [
            c.caseNumber,
            c.title,
        ...
        .slice(0, 10);
      subpoenaResults = readJSON(SUBPOENAS_FILE)
        .filter((s) =>
          [s.subpoenaNumber, s.recipient, s.purpose]
            .join(" ")
            .toLowerCase()
            .includes(lq),
        )
        .slice(0, 5);
      defendantResults = readJSON(DEFENDANTS_FILE)
        .filter((d) =>
          [d.fullName, d.address].join(" ").toLowerCase().includes(lq),
        )
        .slice(0, 5);
    }
    const warrants = readJSON(WARRANTS_FILE);
    const searchableWarrants = hasPerm(pl, "clerk")
      ? warrants
      : warrants.filter((w) => w.status === "active");
    warrantResults = searchableWarrants
      .filter((w) =>
        [w.warrantNumber, w.subject, w.issuedBy, w.judge, w.description]
          .join(" ")
          .toLowerCase()
          .includes(lq),
      )
  ...
  const total =
    ...
    subpoenaResults.length +
    defendantResults.length +
    fileResults.length;
  const body = `
...
app.get("/bar-registry", ensureAuth, (req, res) => {
  ...
  <div class="page-header">
    ...
    <p class="muted-text" style="margin-top:0.25rem">Licensed attorneys and bar members of the State of Texas. (Please note that this document may not be fully up to date.)</p>
  </div>
...
app.get("/admin/roles", requireRealAg, (req, res) => {
  ...
  const LEVEL_META = {
    ag: {
      label: "Attorney General / Administrator",
          ...
          <span class="badge ${badgeClass}" style="display:inline-flex;align-items:center;gap:0.35rem;padding-right:0.3rem">
            ...
            <form method="post" action="/admin/roles/keywords/remove" style="display:inline;margin:0">
              ...
              <button type="submit" title="Remove keyword" style="background:none;border:none;cursor:pointer;color:inherit;padding:0 2px;line-height:1;font-size:0.85rem;opacity:0.7">&times;</button>
                ...
                .join("")
            : '<span class="muted-text" style="font-size:0.85rem">No keywords — all users default to Citizen.</span>'
        }
      </div>
  ...
  const ROLE_OPTIONS = [
    {
      ...
      label: "Citizen",
      badge: "badge-gray",
      desc: "Warrant lookup only",
    },
    {
      value: "clerk",
      label: "Court Clerk",
      badge: "badge-blue",
      desc: "View & edit cases, issue warrants",
    },
    {
      value: "lawyer",
      label: "Attorney / ADA",
      badge: "badge-purple",
      desc: "+ Issue subpoenas",
    },
    {
      value: "ag",
...
app.post("/admin/roles/keywords/add", requireRealAg, (req, res) => {
  ...
  if (!["ag", "lawyer", "clerk"].includes(level))
...
app.post("/admin/view-as", requireRealAg, (req, res) => {
  const { role } = req.body;
  if (!["citizen", "clerk", "lawyer", "ag"].includes(role))
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  if (process.env.DISCORD_BOT_TOKEN && process.env.DISCORD_CLIENT_ID) {
    try {
      ...
      console.error("[DOJ Bot] Failed to start:", err.message);
    }
  } else {
